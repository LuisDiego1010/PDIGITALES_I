...
# Configuración inicial de las paletas (3 píxeles de altura)
...
li s6, 1              # dx (dirección X: 1 = derecha)
li s7, 1              # dy (dirección Y: 1 = abajo)
...
# ================================================================
main_loop:
    # Borrar paleta del Jugador 1 (3 píxeles)
    li t4, -1
erase_p1:
    add t5, t1, t4
    blt t5, zero, erase_p1_next
    bge t5, a2, erase_p1_next
    mul t2, t5, a1        # Dirección = (Y * ancho) + X
    add t2, t2, t0
    slli t2, t2, 2        # Convertir a dirección de word (×4)
    add t2, t2, a0
...
erase_p1_next:
    ...
    # Borrar paleta del Jugador 2 (3 píxeles)
    li t4, -1
erase_p2:
    add t5, s1, t4
...
erase_p2_next:
    ...
    # Borrar pelota libre
    mul t2, s5, a1        # Dirección = (Y * ancho) + X
    add t2, t2, s4
    slli t2, t2, 2
    add t2, t2, a0
    sw t6, 0(t2)
    ...
    # ============================================================
    # Mecánica de colisión optimizada (detectar ANTES de la paleta)
    # ============================================================
    ...
    # Comprobar colisión con Jugador 1 (X=0)
    beq t4, zero, check_p1_collision
    
    # Comprobar colisión con Jugador 2 (X=ancho-1)
    addi t2, a1, -1
...
reverse_dy:
    neg s7, s7                    # Invertir dirección Y
...
reset_ball_center:
    li s4, 17                      # X central
    li s5, 12                      # Y central
    neg s6, s6                     # Invertir dirección X
    li s7, 1                       # Dirección Y inicial

draw_section:
    # Dibujar paleta Jugador 1 (3 píxeles)
    li t4, -1
draw_p1:
    add t5, t1, t4
...
draw_p1_next:
    ...
    # Dibujar paleta Jugador 2 (3 píxeles)
    li t4, -1
draw_p2:
    add t5, s1, t4
...
read_dpad:
    ...
    # Jugador 2: Arriba (0xf0000db4)
    li t4, 0xf0000db4
    lw t5, 0(t4)
    bnez t5, move_up_j2

    # Jugador 2: Abajo (0xf0000db8)
    li t4, 0xf0000db8
    lw t5, 0(t4)
...
# ================================================================
# Movimientos (límites corregidos para 3 píxeles)
# ================================================================
move_up_j1:
    addi t5, t1, -1         # Mover 1 píxel arriba
    li t6, 1                # Límite superior: Y_central >= 1
    blt t5, t6, 1f
    mv t1, t5
1:  jr ra

move_down_j1:
    addi t5, t1, 1          # Mover 1 píxel abajo
    addi t6, a2, -2         # Límite inferior: Y_central <= altura-2
    bge t5, t6, 1f
    mv t1, t5
1:  jr ra
